  ...
  class Plugin
    module Assets
      ...
      def after_initialize
        ...
        Rails.logger.debug { "Registering #{self.assets.count} assets for plugin #{id} precompilation" } if self.assets.present?
      ...
      def find_assets(root)
        return [] unless root.present? && Dir.exists?(root)
      ...
      def assets_from_settings(id)
        Rails.application.initializers.find { |i| i.name.to_s == "#{id}.configure_assets" }.try!(:run)
        assets = SETTINGS[id].try!(:[], :assets).try!(:[], :precompile)
        if assets
          Foreman::Deprecation.deprecation_warning('1.17', "Plugin #{id} must register assets via precompile_assets, not SETTINGS[:#{id}]")
