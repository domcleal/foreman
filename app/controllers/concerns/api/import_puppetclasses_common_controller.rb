module Api::ImportPuppetclassesCommonController
  ...
  api :POST, "/smart_proxies/:smart_proxy_id/environments/:id/import_puppetclasses", N_("Import puppet classes from puppet proxy for particular environment.")
  api :POST, "/environments/:environment_id/smart_proxies/:id/import_puppetclasses", N_("Import puppet classes from puppet proxy for particular environment.")
  ...
  def import_puppetclasses
    ...
    if (errors = ::PuppetClassImporter.new.obsolete_and_new(@changed)).empty?
      ...
      render :json => {:message => N_("Failed to update the environments and puppetclasses from the on-disk puppet installation #{errors.join(", ")}")}, :status => 500
    end
  end
  ...
  def changed_environments
    begin
      ...
      if e.message =~ /puppet feature/i
        msg = N_('We did not find a foreman proxy that can provide the information, ensure that this proxy has the puppet feature turned on.')
      else
        msg = e.message
      end
    ...
    render :json => {:message => N_("No changes to your environments detected")} and return false unless @environments.any?
    @environments.any?
  end
  ...
  def find_required_puppet_proxy
    ...
    unless @smart_proxy && SmartProxy.with_features("Puppet").pluck("smart_proxies.id").include?(@smart_proxy.id)
      not_found N_('We did not find a foreman proxy that can provide the information, ensure that this proxy has the puppet feature turned on.')
    end
    @smart_proxy
  end
