module Api
  module V2
    class HostClassesController < V2::BaseController
      ...
      api :GET, "/hosts/:host_id/puppetclass_ids/", N_("List all puppetclass id's for host")

      def index
        render :json => { root_node_name => HostClass.authorized(:edit_classes).where(:host_id => host_id).pluck('puppetclass_id') }
      end

      api :POST, "/hosts/:host_id/puppetclass_ids", N_("Add a puppetclass to host")
      ...
      api :DELETE, "/hosts/:host_id/puppetclass_ids/:id/", N_("Remove a puppetclass from host")
      param :host_id, String, :required => true, :desc => N_("id of host")
