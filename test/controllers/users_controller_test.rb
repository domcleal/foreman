...
class UsersControllerTest < ActionController::TestCase
  ...
  test "should not ask for confirmation if no password is set" do
    ...
    put :update, params: { :id => user.id, :user => { :login => "foobar" } }, session: set_session_user
  ...
  test 'user with update permission should be able to update another user' do
    setup_user 'edit', 'users'
    put :update, params: { :id => users(:two).id, :user => { :firstname => 'test' } }, session: set_session_user
  ...
  test "#login retains taxonomy session attributes in new session" do
    post :login, params: { :login => {'login' => users(:admin).login, 'password' => 'secret'} }, session: { :location_id => taxonomies(:location1).id, :organization_id => taxonomies(:organization1).id, :foo => 'bar' }
  ...
  context 'default taxonomies' do
    ...
    test 'users can update their own default taxonomies' do
      ...
      put :update, params: { :id   => users(:one).id, :user => { :default_location_id     => taxonomies(:location1).id,
                                :default_organization_id => taxonomies(:organization1).id } }
      assert_redirected_to users_path
