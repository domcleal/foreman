...
class OperatingsystemsControllerTest < ActionController::TestCase
  ...
  context 'os_default_template' do
    ...
    test 'invalid os_default_template should be rejected' do
      ...
      put :update, params: { :id => operatingsystem.id, :operatingsystem => {:os_default_templates_attributes => [{ :provisioning_template_id => nil, :template_kind_id => @template_kind.id }]} }, session: set_session_user
    ...
    test 'empty provisioning_template should be destroyed' do
      ...
      assert_difference 'OsDefaultTemplate.count', -1 do
        ...
        put :update, params: { :id => operatingsystem.id, :operatingsystem => {:os_default_templates_attributes => [{ :id => os_default_template_id, :provisioning_template_id => '', :template_kind_id => @template_kind.id }]} }, session: set_session_user
      end
    end
  end
end
