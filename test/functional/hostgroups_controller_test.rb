...
class HostgroupsControllerTest < ActionController::TestCase
  ...
  test 'owners of a hostgroup up in the hierarchy get ownership of all children' do
    ...
    post :create, {"hostgroup" => {"name"=>"first" , "parent_id"=> Hostgroup.find_by_name("root").id}}, set_session_user.merge(:user => @one.id)
    assert_response :redirect

    post :create, {"hostgroup" => {"name"=>"second", "parent_id"=> Hostgroup.find_by_name("first").id}}, set_session_user.merge(:user => @one.id)
